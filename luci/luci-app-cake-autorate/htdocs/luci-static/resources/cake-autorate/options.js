...
var OPTIONS = [
	...
	{ name: 'min_dl_shaper_rate_kbps', group: 'essentials', type: 'integer', lo: 1,
	  desc: 'Floor for the download shaper rate.', units: 'Kbit/s, must not be above the base download rate' },
	{ name: 'base_dl_shaper_rate_kbps', group: 'essentials', type: 'integer', lo: 1,
	  ...
	  units: 'Kbit/s, between the minimum and maximum download rates' },
	{ name: 'max_dl_shaper_rate_kbps', group: 'essentials', type: 'integer', lo: 1,
	  desc: 'Ceiling for the download shaper rate.', units: 'Kbit/s, must not be below the base download rate' },
	{ name: 'min_ul_shaper_rate_kbps', group: 'essentials', type: 'integer', lo: 1,
	  desc: 'Floor for the upload shaper rate.', units: 'Kbit/s, must not be above the base upload rate' },
	{ name: 'base_ul_shaper_rate_kbps', group: 'essentials', type: 'integer', lo: 1,
	  ...
	  units: 'Kbit/s, between the minimum and maximum upload rates' },
	{ name: 'max_ul_shaper_rate_kbps', group: 'essentials', type: 'integer', lo: 1,
	  desc: 'Ceiling for the upload shaper rate.', units: 'Kbit/s, must not be below the base upload rate' },
	...
	{ name: 'shaper_rate_max_adjust_down_bufferbloat', group: 'shaper', type: 'float', lo: 0, hi: 1,
	  desc: 'Largest shaper-rate reduction factor, applied at the average OWD delta threshold.', units: 'multiplier between 0 and 1', doc: DOC },
	{ name: 'shaper_rate_adjust_up_load_high', group: 'shaper', type: 'float', lo: 1,
	  desc: 'Shaper-rate increase factor while load is high and no bufferbloat is seen.', units: 'multiplier of 1 or more' },
	{ name: 'shaper_rate_adjust_down_load_low', group: 'shaper', type: 'float', lo: 0, hi: 1,
	  desc: 'Decay factor bringing an above-base rate back down toward base on low/idle load.', units: 'multiplier between 0 and 1' },
	{ name: 'shaper_rate_adjust_up_load_low', group: 'shaper', type: 'float', lo: 1,
	  desc: 'Decay factor bringing a below-base rate back up toward base on low/idle load.', units: 'multiplier of 1 or more' },
	{ name: 'high_load_thr', group: 'shaper', type: 'float', lo: 0, hi: 1,
	  desc: 'Achieved-rate fraction of the current shaper rate above which load is classified high.', units: 'fraction between 0 and 1' },
	...
	{ name: 'no_pingers', group: 'pingers', type: 'integer', lo: 1,
	  desc: 'Number of concurrent pinger processes / live reflectors.', units: 'a count of at least 1, and no more than the number of reflectors' },
	...
	{ name: 'reflector_misbehaving_detection_thr', group: 'reflectors', type: 'integer', lo: 1,
	  desc: 'Offences within the window before a reflector is deemed misbehaving and replaced.', units: 'a count of offences, no more than the detection window' },
	...
	{ name: 'bufferbloat_detection_window', group: 'detection', type: 'integer', lo: 1,
	  desc: 'Number of recent delay samples retained in the detection window.', units: 'a count of samples, more than the bufferbloat detection threshold' },
	{ name: 'bufferbloat_detection_thr', group: 'detection', type: 'integer', lo: 1,
	  desc: 'Number of delayed samples within the window that triggers a bufferbloat event.', units: 'a count of samples, no more than the detection window' },
	{ name: 'alpha_baseline_increase', group: 'detection', type: 'float', lo: 0, hi: 1,
	  desc: 'EWMA alpha for how rapidly the OWD baseline may rise (kept slow so path changes track without masking bufferbloat).', units: 'an EWMA alpha between 0 and 1', doc: DOC },
	{ name: 'alpha_baseline_decrease', group: 'detection', type: 'float', lo: 0, hi: 1,
	  desc: 'EWMA alpha for how rapidly the OWD baseline may fall (kept fast to track the shortest path).', units: 'an EWMA alpha between 0 and 1', doc: DOC },
	{ name: 'alpha_delta_ewma', group: 'detection', type: 'float', lo: 0, hi: 1,
	  desc: 'Smoothing factor applied to the OWD delta from baseline.', units: 'an EWMA alpha between 0 and 1', doc: DOC },
	...
	{ name: 'startup_wait_s', group: 'idle', type: 'float', lo: 0,
	  desc: 'Delay before the daemon starts work, letting interfaces settle after a reboot.', units: 'seconds, 0 or more' },
	...
	{ name: 'log_file_max_size_KB', group: 'logging', type: 'integer', lo: 1,
	  desc: 'Rotate the log file once this many KB of log lines have accumulated.', units: 'KB, more than 0' },
