 ...
 *
 * The live status view is a separate view (view/cake-autorate/status.js, task 9).
 * This file is config editing PLUS the SQM-validated dl_if / ul_if pickers (task
 * 9 component C): both are turned into comboboxes populated from the task-8
 * `sqm_interfaces` rpcd method and carry a visible, NON-blocking warning when the
 * chosen interface is not backed by a live SQM CAKE qdisc. Version shown is the
 * PACKAGE version (3.2.2), not the daemon's stale self-reported 3.2.1 (upstream
 * cosmetic bug at tag v3.2.2).
 *
...
/* task-8 rpcd: derive the shaping interfaces from the LIVE SQM config so dl_if /
 * ul_if become validated choices instead of free text. */
...
/*
 * Refresh the inline SQM-validation notice under one dl_if/ul_if widget for a
 * given section. Non-blocking: the value is never rejected, only annotated, so
 * an existing value still displays even when SQM is not configured yet. The
 * decision itself (live.interfaceStatus) is pure and unit-tested.
 */
...
return view.extend({
	...
	renderForm: function (sqm) {
		...
		options.OPTIONS.forEach(function (opt) {
			...
			/* Component C: the two interface fields become SQM-validated
			 * comboboxes -- free text is still allowed (so a value set before SQM
			 * exists keeps displaying) but the SQM-derived devices are offered as
			 * choices and a mismatch raises a visible, non-blocking warning. */
		...
		return m.render().then(function (mapEl) {
			...
			/* Component C: tell the user what the interface fields validate
			 * against, so an empty/absent SQM config is not mistaken for a bug. */
