...
> **On a 5G/LTE/WISP link, read [`tuning.md`](tuning.md) before touching
> **Delay & bufferbloat detection**.** Much of the loaded latency on a radio
> uplink is the RAN scheduler, downstream of your shaper. Set
> `ul_owd_delta_thr_ms` below that floor and the daemon detects bufferbloat
> forever, cutting the shaper to `min_ul_shaper_rate_kbps` within seconds and
> never recovering. The package installs `cake-autorate-probe` for exactly this
> measurement.
